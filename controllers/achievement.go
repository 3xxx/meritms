...
//根据权限，显示侧栏结构，方便查看科室里员工总体情况，以及查看员工个人详细
func (c *Achievement) GetAchievement() {
	...
	for _, v := range roles {
		//由角色取出科室id
		secid := e.GetFilteredPolicy(0, v, "secoffice")
		// GetPermissionsForUser只能过滤一个字段，所以不行
		//为了去重，使用map
		for _, v1 := range secid {
			l := len(tempMap)
			secid_1 := strings.Replace(v1[2], "secoffice_", "", -1)
			tempMap[secid_1] = 0
			if len(tempMap) != l { // 加入map后，map长度变化，则元素不重复
				secids = append(secids, secid_1)
			}
		}
	}
	...
	var depcount int
	var departid int //部门数组的序号
	//循环科室id——如果科室id的parentid为0，则说明是部门，在显示这个部门下所有科室
	//如果科室id的parentid不等于0，则在secids查找parentid，如果有，则跳过；如果没有，则显示科室。
	var treeview, hasdepart bool
	if role == "1" { //如果是管理员，则显示全部
	...
	} else if len(secids) > 0 { //如果不是管理员，则根据权限显示侧栏
		for _, v2 := range secids {
			// beego.Info(v2)
			//取得科室（可能是部门）
			secidnum, err := strconv.ParseInt(v2, 10, 64)
			category, err := models.GetAdminDepartbyId(secidnum)
			if err != nil {
				beego.Error(err)
			}
			if category.ParentId == 0 { //说明是部门id
				achsecoffice = make([]AchSecoffice, 0) //把slice置0——坑
				//取出部门下所有科室
				category2, err := models.GetAdminDepart(category.Id) //得到多个科室
				if err != nil {
					beego.Error(err)
				}
				aa := make([]AchDepart, 1)
				...
				if len(category2) > 0 {
					for i2, _ := range category2 {
						...
						bb[0].Id = category2[i2].Id
						bb[0].Level = "2"
						bb[0].Pid = category.Id
						bb[0].Title = category2[i2].Title //科室名称
						// beego.Info(category2[i2].Title)
						//根据分院和科室查所有员工
						users, count, err := models.GetUsersbySec(category.Title, category2[i2].Title) //得到员工姓名
						if err != nil {
							beego.Error(err)
						}
						for i3, _ := range users {
				...
				//根据分院查所有员工
				// beego.Info(category1[i1].Title)
				users, count, err := models.GetUsersbySecOnly(category.Title) //得到员工姓名
				if err != nil {
					beego.Error(err)
				}
				// beego.Info(users)
				...
				treeview = false //这个应该要吧，坑
				for _, v3 := range secids {
					if strconv.FormatInt(category.ParentId, 10) == v3 {
						//存在上级部门，则跳过
						treeview = true
					}
				}
				if !treeview {
					//查出部门
					depart, err := models.GetAdminDepartbyId(category.ParentId)
					if err != nil {
						beego.Error(err)
					}
					// beego.Info(depart.Title)
					//根据科室id查所有员工
					users, count, err := models.GetUsersbySecId(v2) //得到员工姓名
					if err != nil {
						beego.Error(err)
					}
					bb := make([]AchSecoffice, 1) //科室
					bb[0].Id = category.Id
					bb[0].Level = "2"
					bb[0].Pid = category.ParentId
					bb[0].Title = category.Title //科室名称
					for i3, _ := range users {
						cc := make([]AchEmployee, 1)
						cc[0].Id = users[i3].Id
						cc[0].Level = "3"
						cc[0].Pid = category.Id
						cc[0].Nickname = users[i3].Nickname //名称
						achemployee = append(achemployee, cc...)
					}
					bb[0].Tags[0] = strconv.Itoa(count)
					...
					if hasdepart { //将上面科室追加到数据结构里的部门中
						achsecoffice = append(achsecoffice, bb...)
						achdepart[departid].Secoffice = achsecoffice
					} else { //如果部门不存在，则查出部门，将科室加进去部门里，在将部门追加到部门结构数据里
						achsecoffice = make([]AchSecoffice, 0) //再把slice置0——坑
						aa := make([]AchDepart, 1)
						aa[0].Id = depart.Id
						aa[0].Level = "1"
						// aa[0].Pid = category[0].Id
...
func (c *Achievement) SendCatalog() {
	...
	for _, v := range ob {
		...
		case 1:
			//制图者和设计者不同名，则提交给设计
			if catalog.Designd != "" && catalog.Designd != catalog.Drawn {
			...
			} else if catalog.Designd != "" && catalog.Designd == catalog.Drawn {
